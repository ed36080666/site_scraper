<template>
  ...
  >
    <td class="pl-0 py-2">
      ...
      />
      <span v-if="showMissingFileIcon" class="text-amber-500" style="position: absolute; transform: translateX(-90px);" title="File missing">
...
export default {
  ...
  computed: {
    showMissingFileIcon() {
      return !this.item.file_exists && this.item.status !== 'queued' && this.item.status !== 'processing';
    }
  },
  methods: {
